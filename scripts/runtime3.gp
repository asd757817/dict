...
plot [:][:] 'bench_cpy.txt' using 1:2 with points lc rgb "blue" title 'cpy 
###
set xlabel 'experiment'
set ylabel 'time(sec)'
...
set format x "%5.0f"
set xtics rotate by 45 right
plot [:][:] 'bench_ref.txt' using 1 with points lc rgb "red" title 'ref'
...
plot [:][:] 'bench_cpy.txt' using 1:2 with points lc rgb "blue" title 'cpy',\
				 'bench_ref.txt' using 1 with points lc rgb "red" title 'ref',\
